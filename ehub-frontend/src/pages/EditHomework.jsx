...
const EditHomework = () => {
    ...
    const loadHomework = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          setDueDate(homework.dueDate ? homework.dueDate.split('T')[0] : '');
          setLevel(homework.level || '');
          setLanguage(homework.language || 'english');
          
          // Load interactive elements
          const elements = homework.extractedQuestions || homework.interactiveElements || [];
          setInteractiveElements(elements.map(element => ({
            ...element,
            id: element.id || Date.now().toString() + Math.random()
          })));
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update homework in backend
      const response = await homeworkApi.updateHomework(id, homeworkData);
      
      if (response.data.success) {
        console.log('Homework updated successfully:', response.data);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 pt-16">
      <div className="flex items-center mb-6">
        <button 
          onClick={() => navigate('/homework/manage')}
        ...
        <h1 className="text-3xl font-bold text-gray-800">Edit Interactive Homework</h1>
      </div>
