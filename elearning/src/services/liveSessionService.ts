...
export const liveSessionService = {
  ...
  uploadRecording: async (id: string, videoFile: File): Promise<ILiveSession> => {
    ...
    const response = await apiService.postFormData<{ session: ILiveSession }>(`/teacher/live-sessions/${id}/upload-recording`, formData);
  ...
  getCourseSessions: async (courseId: string): Promise<ILiveSession[]> => {
    try {
      const response = await apiService.get<{ sessions: ILiveSession[] }>(`/live-sessions/course/${courseId}`);
      
      if (response.success && response.data) {
        return response.data.sessions || [];
      }
      
      throw new Error(response.error || 'Failed to fetch course sessions');
    } catch (error: any) {
