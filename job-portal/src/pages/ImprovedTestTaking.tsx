...
const ImprovedTestTaking: React.FC = () => {
    ...
    const initializeTest = async () => {
      try {
        ...
        if (location.state && location.state.testData) {
          ...
          // Try to get session from URL params or stored data
  ...
  const handleSubmitTest = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/psychometric-tests/submit/${testData.sessionId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
...
export default ImprovedTestTaking;